...
RUN sed -i -e 's/delete/del/g' njs/src/njs.h
WORKDIR /home/build/njs
RUN CFLAGS="-DNJS_HAVE_POSIX_MEMALIGN=1 -fsanitize=address" ./configure && make
RUN find /home/build/njs -iname nxt*.h -or -iname njs*.h -exec cp {} /usr/include \;
WORKDIR /home/build/
